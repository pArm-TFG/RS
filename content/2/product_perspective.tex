...
Dado que el código de control y de cálculo matricial será cargado en su totalidad dentro de \ac{S2}, se han de tener en cuenta las restricciones de memoria del microcontrolador utilizado.

Debido al microcontrolador elegido por el equipo de desarrollo, se plantean las siguientes restricciones de memoria:
...
Para poder lograrlo, el equipo de desarrollo realizará labores de refactorización, las cuales buscan poder optimizar el código. Dicha optimización supone investigar e implementar algoritmos más eficientes a los ya existentes.

Además, se eliminará todo el código duplicado y se buscará crear un código más genérico, disminuyendo de esta manera el número de métodos y reduciendo el tamaño del código.

El equipo de desarrollo ha creído conveniente centrar sus esfuerzos en crear un código óptimo en memoria ya que este es un limitante clave en sistemas empotrados dado que la cantidad de la que disponen es fija y difícilmente ampliable.
